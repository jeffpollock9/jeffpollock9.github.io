...
One of the first things to consider doing is a [[https://betanalpha.github.io/assets/case_studies/principled_bayesian_workflow.html#113_prior_predictive_checks][prior predictive check]]. This involved
...
We later show plots which make use of these values.y

The ~unnest~ module isn't part of the public TFP API - but it does make writing this
function a lot easier.

We need to initialise the MCMC routine in some certain state. when doing our prior
predictive check we actually sampled many times from the prior, so one way of
initialising the MCMC routine is to use some of those (one for each chain):
...
defined in my joint distribution, so I resort to usin a private member function to get
